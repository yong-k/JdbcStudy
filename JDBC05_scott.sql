...
---------------------------------내가 풀면서 쓴 거--------------------------------------
-- 쿼리문
--1. 직원 정보 입력   *city_id, buseo_id, jikwi_id 입력은 한글로 하고 저장은 번호로 *salary
INSERT INTO TBL_EMP(EMP_ID, EMP_NAME, SSN, IBSADATE, CITY_ID, TEL, BUSEO_ID, JIKWI_ID, BASICPAY, SUDANG) VALUES(EMPSEQ.NEXTVAL, '%s', '%s', '%s', %d, '%s', %d, %d, %d, %d)
;
--지역번호
...
--2. 직원 전체 출력
--1) 사번 정렬
SELECT E.EMP_ID, E.EMP_NAME, E.SSN, E.IBSADATE, C.CITY_NAME, E.TEL, B.BUSEO_NAME, J.JIKWI_NAME, E.BASICPAY, E.SUDANG, (NVL(E.BASICPAY, 0) + NVL(E.SUDANG, 0)) AS SALARY FROM TBL_EMP E JOIN TBL_CITY C ON E.CITY_ID = C.CITY_ID JOIN TBL_BUSEO B ON E.BUSEO_ID = B.BUSEO_ID JOIN TBL_JIKWI J ON E.JIKWI_ID = J.JIKWI_ID ORDER BY EMP_ID
;

--2) 이름 정렬
SELECT E.EMP_ID, E.EMP_NAME, E.SSN, E.IBSADATE, C.CITY_NAME, E.TEL, B.BUSEO_NAME, J.JIKWI_NAME, E.BASICPAY, E.SUDANG, (NVL(E.BASICPAY, 0) + NVL(E.SUDANG, 0)) AS SALARY FROM TBL_EMP E JOIN TBL_CITY C ON E.CITY_ID = C.CITY_ID JOIN TBL_BUSEO B ON E.BUSEO_ID = B.BUSEO_ID JOIN TBL_JIKWI J ON E.JIKWI_ID = J.JIKWI_ID ORDER BY EMP_NAME
;

--3) 부서 정렬
SELECT E.EMP_ID, E.EMP_NAME, E.SSN, E.IBSADATE, C.CITY_NAME, E.TEL, B.BUSEO_NAME, J.JIKWI_NAME, E.BASICPAY, E.SUDANG, (NVL(E.BASICPAY, 0) + NVL(E.SUDANG, 0)) AS SALARY FROM TBL_EMP E JOIN TBL_CITY C ON E.CITY_ID = C.CITY_ID JOIN TBL_BUSEO B ON E.BUSEO_ID = B.BUSEO_ID JOIN TBL_JIKWI J ON E.JIKWI_ID = J.JIKWI_ID ORDER BY BUSEO_NAME
;
...
--5) 급여 내림차순 정렬
SELECT E.EMP_ID, E.EMP_NAME, E.SSN, E.IBSADATE, C.CITY_NAME, E.TEL, B.BUSEO_NAME, J.JIKWI_NAME, E.BASICPAY, E.SUDANG, (NVL(E.BASICPAY, 0) + NVL(E.SUDANG, 0)) AS SALARY FROM TBL_EMP E JOIN TBL_CITY C ON E.CITY_ID = C.CITY_ID JOIN TBL_BUSEO B ON E.BUSEO_ID = B.BUSEO_ID JOIN TBL_JIKWI J ON E.JIKWI_ID = J.JIKWI_ID ORDER BY SALARY DESC
;


--3. 직원 검색 출력
--1) 사번 검색
SELECT EMP_ID, EMP_NAME, SSN, IBSADATE, CITY_NAME, TEL, BUSEO_NAME, JIKWI_NAME, BASICPAY, SUDANG, SALARY FROM (SELECT E.EMP_ID, E.EMP_NAME, E.SSN, E.IBSADATE, C.CITY_NAME, E.TEL, B.BUSEO_NAME, J.JIKWI_NAME, E.BASICPAY, E.SUDANG, (NVL(E.BASICPAY, 0) + NVL(E.SUDANG, 0)) AS SALARY FROM TBL_EMP E JOIN TBL_CITY C ON E.CITY_ID = C.CITY_ID JOIN TBL_BUSEO B ON E.BUSEO_ID = B.BUSEO_ID JOIN TBL_JIKWI J ON E.JIKWI_ID = J.JIKWI_ID ORDER BY EMP_ID) WHERE EMP_ID = %d
;

--2) 이름 검색
SELECT EMP_ID, EMP_NAME, SSN, IBSADATE, CITY_NAME, TEL, BUSEO_NAME, JIKWI_NAME, BASICPAY, SUDANG, SALARY FROM (SELECT E.EMP_ID, E.EMP_NAME, E.SSN, E.IBSADATE, C.CITY_NAME, E.TEL, B.BUSEO_NAME, J.JIKWI_NAME, E.BASICPAY, E.SUDANG, (NVL(E.BASICPAY, 0) + NVL(E.SUDANG, 0)) AS SALARY FROM TBL_EMP E JOIN TBL_CITY C ON E.CITY_ID = C.CITY_ID JOIN TBL_BUSEO B ON E.BUSEO_ID = B.BUSEO_ID JOIN TBL_JIKWI J ON E.JIKWI_ID = J.JIKWI_ID ORDER BY EMP_ID) WHERE EMP_NAME = '%s'
;

--3) 부서 검색
SELECT EMP_ID, EMP_NAME, SSN, IBSADATE, CITY_NAME, TEL, BUSEO_NAME, JIKWI_NAME, BASICPAY, SUDANG, SALARY FROM (SELECT E.EMP_ID, E.EMP_NAME, E.SSN, E.IBSADATE, C.CITY_NAME, E.TEL, B.BUSEO_NAME, J.JIKWI_NAME, E.BASICPAY, E.SUDANG, (NVL(E.BASICPAY, 0) + NVL(E.SUDANG, 0)) AS SALARY FROM TBL_EMP E JOIN TBL_CITY C ON E.CITY_ID = C.CITY_ID JOIN TBL_BUSEO B ON E.BUSEO_ID = B.BUSEO_ID JOIN TBL_JIKWI J ON E.JIKWI_ID = J.JIKWI_ID ORDER BY EMP_ID) WHERE BUSEO_NAME = '%s'
;

--4) 직위 검색
SELECT EMP_ID, EMP_NAME, SSN, IBSADATE, CITY_NAME, TEL, BUSEO_NAME, JIKWI_NAME, BASICPAY, SUDANG, SALARY FROM (SELECT E.EMP_ID, E.EMP_NAME, E.SSN, E.IBSADATE, C.CITY_NAME, E.TEL, B.BUSEO_NAME, J.JIKWI_NAME, E.BASICPAY, E.SUDANG, (NVL(E.BASICPAY, 0) + NVL(E.SUDANG, 0)) AS SALARY FROM TBL_EMP E JOIN TBL_CITY C ON E.CITY_ID = C.CITY_ID JOIN TBL_BUSEO B ON E.BUSEO_ID = B.BUSEO_ID JOIN TBL_JIKWI J ON E.JIKWI_ID = J.JIKWI_ID ORDER BY EMP_ID) WHERE JIKWI_NAME = '%s'
;


--4. 직원 정보 수정  *salary
UPDATE TBL_EMP SET EMP_NAME = '%s', SSN = '%s', IBSADATE = '%s', CITY_ID = %d, TEL = '%s', BUSEO_ID = %d, JIKWI_ID = %d, BASICPAY = %d, SUDANG = %d WHERE EMP_ID = %d
;

--조인(지역,부서,직위)
SELECT E.EMP_ID, C.CITY_ID, C.CITY_NAME, B.BUSEO_ID, B.BUSEO_NAME, J.JIKWI_ID, J.JIKWI_NAME
...
UPDATE TBL_EMP               
SET EMP_NAME = '%s', SSN = '%s', IBSADATE = '%s', CITY_ID = (SELECT CITY_ID FROM TBL_CITY WHERE CITY_NAME = '강원'), TEL = '%s', BUSEO_ID = (SELECT BUSEO_ID FROM TBL_BUSEO WHERE BUSEO_NAME = '개발부'), JIKWI_ID = (SELECT JIKWI_ID FROM TBL_JIKWI WHERE JIKWI_NAME = '사장'), BASICPAY = %d, SUDANG = %d
WHERE EMP_ID = %d;            
           
UPDATE TBL_EMP               
SET CITY_ID = (SELECT CITY_ID FROM TBL_CITY WHERE CITY_NAME = '강원')
  , BUSEO_ID = (SELECT BUSEO_ID FROM TBL_BUSEO WHERE BUSEO_NAME = '개발부')
  , JIKWI_ID = (SELECT JIKWI_ID FROM TBL_JIKWI WHERE JIKWI_NAME = '사장')
WHERE EMP_ID = 1001;

SELECT * FROM TBL_EMP;
select * from tbl_city;     -- 6.서울   -> 1.강원
select * from tbl_buseo;    -- 2.기획부 -> 1.개발부
select * from tbl_jikwi;    -- 5.부장   -> 1.사장

--최종
UPDATE TBL_EMP SET EMP_NAME = '%s', SSN = '%s', IBSADATE = '%s', CITY_ID = (SELECT CITY_ID FROM TBL_CITY WHERE CITY_NAME = '강원'), TEL = '%s', BUSEO_ID = (SELECT BUSEO_ID FROM TBL_BUSEO WHERE BUSEO_NAME = '개발부'), JIKWI_ID = (SELECT JIKWI_ID FROM TBL_JIKWI WHERE JIKWI_NAME = '사장'), BASICPAY = %d, SUDANG = %d WHERE EMP_ID = %d
; 


--5. 직원 정보 삭제
DELETE TBL_EMP WHERE EMP_ID = %d
;

--메뉴 출력
--지역
SELECT LISTAGG(CITY_NAME, '/') WITHIN GROUP(ORDER BY CITY_ID) AS LIST FROM TBL_CITY
;
--부서
SELECT LISTAGG(BUSEO_NAME, '/') WITHIN GROUP(ORDER BY BUSEO_ID) AS LIST FROM TBL_BUSEO
;
--직위
SELECT LISTAGG(JIKWI_NAME, '/') WITHIN GROUP(ORDER BY JIKWI_ID) AS LIST FROM TBL_JIKWI
;
--기본급
SELECT MIN_BASICPAY FROM TBL_JIKWI WHERE JIKWI_NAME = '%s'
;
...
UPDATE TBL_EMP
SET EMP_NAME = '홍길동', SSN = '771212-1022432', IBSADATE = '1998-10-11', CITY_ID = 6
, TEL = '011-2356-4528', BUSEO_ID = 2, JIKWI_ID = 5, BASICPAY = 2610000, SUDANG = 200000
WHERE EMP_ID = 1001;
--홍길동	771212-1022432	1998-10-11	6	011-2356-4528	2	5	2610000	200000
commit;
...
-- OUTER JOIN 했을 경우 데이터 일치하지 않을 경우 있기 때문에,
-- 데이터 건수 꼭 확인해볼 것 !!
-- ※ SCOTT 에 KING, HR의 KIMBERLY 안 나왔던 것 처럼
...
-- 9. 직원 전체 출력 쿼리문 (사번/이름/부서/직위/급여내림차순 정렬)
-- 사번  이름  주민번호  입사일  지역  전화번호  부서  직위  기본급  수당  급여
